...
#           prob[2] = np.exp( (g[i][j][0] - g[i][j][5])/ (2*(g[i][j][2]-g[i][j][1])))
#           prob[2] = prob[2] / ( np.exp( (g[i][j][0] - g[i][j][3])/ (2*(g[i][j][2]-g[i][j][1]))) +  np.exp( (g[i][j][0] - g[i][j][4])/ (2*(g[i][j][2]-g[i][j][1]))) + np.exp( (g[i][j][0] - g[i][j][5])/ (2*(g[i][j][2]-g[i][j][1]))))
...
#og.update({splitline[0]:None})
#og.update({splitline[1]:None})
...
#    og[splitline[1]].update({splitline[0]:[float(splitline[2]),float(splitline[3]),float(splitline[4])]})
#    noiseadded[splitline[0]].update({splitline[1]:[float(splitline[2]),float(splitline[3]),float(splitline[4])]})
...
#    noiseadded[splitline[1]].update({splitline[0]:[float(splitline[2]),float(splitline[3]),float(splitline[4])]})
...
#a,b = Dijkstra(og,og,source,destination)
...
print(noiseadded)
#f=open(landscape, 'r')
#noiseadded = Exponential(ex,og,float(Ipsilon))
